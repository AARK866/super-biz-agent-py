...
REM 避免系统级 DEBUG（例如 DEBUG=release）覆盖项目 .env 中的布尔配置
set "DEBUG="
set "MILVUS_HOST="
set "MILVUS_PORT="
...
echo [信息] 等待 Milvus 健康检查通过（最多 180 秒）...
set "MILVUS_HEALTH="
set /a MILVUS_WAITED=0

:wait_milvus
for /f "delims=" %%s in ('docker inspect --format "{{.State.Health.Status}}" superbiz-milvus-2611 2^>nul') do set "MILVUS_HEALTH=%%s"
if /i "!MILVUS_HEALTH!"=="healthy" goto :milvus_ready
if !MILVUS_WAITED! GEQ 180 (
    echo [错误] Milvus 在 180 秒内未就绪，当前状态: !MILVUS_HEALTH!
    docker compose -f vector-database.yml ps
    pause
    exit /b 1
)
timeout /t 5 /nobreak >nul
set /a MILVUS_WAITED+=5
goto :wait_milvus
