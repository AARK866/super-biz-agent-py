...
DOCS_DIR = aiops-docs
MILVUS_CONTAINER = superbiz-milvus-2611
COMPOSE = docker compose -f vector-database.yml
...
init:
	...
	@echo "   Attu (可选，需 --profile ui): http://localhost:8001"
	@echo "   MinIO: http://localhost:19001 (admin/minioadmin)"
	@echo ""
...
up:
	...
	@echo "$(YELLOW)🚀 启动项目专用 Milvus 2.6.11 容器...$(NC)"
	@$(COMPOSE) up -d
	@echo "$(YELLOW)⏳ 等待 Milvus 健康检查通过（最多 180 秒）...$(NC)"
	@max_attempts=36; \
	attempt=0; \
	while [ "$$(docker inspect --format '{{.State.Health.Status}}' $(MILVUS_CONTAINER) 2>/dev/null)" != "healthy" ]; do \
		attempt=$$((attempt + 1)); \
		if [ $$attempt -ge $$max_attempts ]; then \
			echo "$(RED)❌ Milvus 启动超时$(NC)"; \
			$(COMPOSE) ps; \
			exit 1; \
		fi; \
		sleep 5; \
	...
	@echo "   Milvus 健康检查: http://localhost:9092/healthz"
	@echo "   Attu: 可选组件，运行 $(COMPOSE) --profile ui up -d 后访问 http://localhost:8001"
	@echo "   MinIO: http://localhost:19001 (admin/minioadmin)"
...
down:
	...
	@$(COMPOSE) down
	@echo "$(GREEN)✅ Docker 容器已停止（命名卷数据已保留）$(NC)"
